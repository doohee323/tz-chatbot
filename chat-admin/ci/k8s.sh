...
build_frontend() {
    ...
    [[ -f "env-frontend" ]] && sed -i.bak "s/BASE_DOMAIN_PLACEHOLDER/${BASE_DOMAIN}/g" env-frontend && rm -f env-frontend.bak
...
deploy_to_kubernetes() {
    ...
    # Save raw POSTGRES_PASSWORD for DB create/migrate (before base64 overwrite)
    DB_PASS_RAW="${POSTGRES_PASSWORD}"
    ...
    # TZ-Chat secrets (base64 for k8s Secret data)
    ...
    # Ensure chat_admin database exists (like tz-cointutor migration)
    if [ -n "${DB_PASS_RAW}" ]; then
        log_info "🔄 Ensuring chat_admin database exists..."
        POSTGRES_NS="${NAMESPACE}"
        POSTGRES_NAME="devops-postgres-postgresql"
        POSTGRES_POD=""
        if kubectl -n ${POSTGRES_NS} get deploy ${POSTGRES_NAME} >/dev/null 2>&1; then
            POSTGRES_POD="deploy/${POSTGRES_NAME}"
        elif kubectl -n ${POSTGRES_NS} get statefulset ${POSTGRES_NAME} >/dev/null 2>&1; then
            POSTGRES_POD="${POSTGRES_NAME}-0"
        fi
        if [ -n "${POSTGRES_POD}" ]; then
            if kubectl -n ${POSTGRES_NS} exec ${POSTGRES_POD} -- env PGPASSWORD="${DB_PASS_RAW}" psql -U postgres -c "SELECT 1 FROM pg_database WHERE datname='chat_admin'" -tA 2>/dev/null | grep -q 1; then
                log_info "  Database chat_admin already exists"
            else
                kubectl -n ${POSTGRES_NS} exec ${POSTGRES_POD} -- env PGPASSWORD="${DB_PASS_RAW}" psql -U postgres -c "CREATE DATABASE chat_admin" && log_info "  Created database chat_admin" || log_warning "  Failed to create database chat_admin (may already exist)"
            fi
        else
            log_warning "  Postgres ${POSTGRES_NAME} not found in ${POSTGRES_NS}, skipping DB create"
        fi
    else
        log_warning "  POSTGRES_PASSWORD not set, skipping database creation"
    fi
    ...
    if [ "${ARGOCD_ENABLED}" = "true" ]; then
        ...
        # Determine deployment name by branch (chat-admin, chat-admin-qa, chat-admin-{branch})
        if [ "${GIT_BRANCH}" = "main" ]; then
            DEPLOYMENT_NAME="chat-admin"
        else
            DEPLOYMENT_NAME="chat-admin-${SECRET_SUFFIX}"
        fi

        # Run migration (init_db) BEFORE rollout - like tz-cointutor.
        # chat-admin needs tables to start (refresh_systems_cache in lifespan), so run init_db first.
        DB_PASSWORD=$(kubectl -n ${NAMESPACE} get secret chat-admin-secret-${SECRET_SUFFIX} -o jsonpath='{.data.POSTGRES_PASSWORD}' | base64 -d)
        if [ -z "${DB_PASSWORD}" ]; then
            log_error "❌ Cannot get database password from chat-admin-secret-${SECRET_SUFFIX}."
            exit 1
        fi
