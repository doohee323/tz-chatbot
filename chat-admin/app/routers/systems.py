...
@router.post("/apply-settings", response_model=dict)
async def apply_settings(
    _admin: str = Depends(get_admin_required),
):
    """Restart chat-gateway deployment so it picks up DB changes. Waits for rollout to complete."""
    deployment = os.environ.get("APPLY_SETTINGS_DEPLOYMENT", "chat-gateway")
    namespace = os.environ.get("APPLY_SETTINGS_NAMESPACE", "devops")
    env = os.environ.copy()
    try:
        r = subprocess.run(
        ...
        if r.returncode != 0:
            ...
            raise HTTPException(
                ...
                detail=f"kubectl rollout restart failed: {err}",
            )
        r2 = subprocess.run(
        ...
        if r2.returncode != 0:
            err = r2.stderr.strip() or r2.stdout.strip() or "Rollout status timeout or error"
            raise HTTPException(
                status_code=status.HTTP_502_BAD_GATEWAY,
                detail=f"Rollout not ready: {err}",
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=f"apply-settings failed: {e}",
        ) from e
    return {"status": "ok", "deployment": deployment, "namespace": namespace}
