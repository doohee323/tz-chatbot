...
chat-gateway는 두 가지 Dify 구성을 지원합니다:

1. **공용 Dify** (기본): 모든 앱이 같은 Dify 사용
   ```bash
   DIFY_BASE_URL="https://dify.example.com"
   DIFY_API_KEY="your-api-key"
   ```

2. **시스템별 Dify** (선택): DrillQuiz·CoinTutor 등 각각 다른 Dify 사용 가능
   ```bash
   DIFY_DRILLQUIZ_BASE_URL="https://dify-drillquiz.example.com"
   DIFY_DRILLQUIZ_API_KEY="drillquiz-key"

   DIFY_COINTUTOR_BASE_URL="https://dify-cointutor.example.com"
   DIFY_COINTUTOR_API_KEY="cointutor-key"

   # 위에서 지정하지 않은 앱은 기본값 사용
   DIFY_BASE_URL="https://dify-default.example.com"
   DIFY_API_KEY="default-key"
   ```

K8s ConfigMap에서 이 환경변수들을 설정하면, 각 system_id 요청 시 자동으로 해당 Dify를 선택합니다.
