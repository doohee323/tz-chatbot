...
**Dify 내장 RAG 한계·강의 관점**  
• 내장 RAG는 초기 구축에 유리하나, 검색 품질·파이프라인 제어·복수 앱 분리 등 한계 → 실무에서 LangChain 자체 RAG 전환 많음  
• 이 강의: **Dify로 RAG 흐름 이해** + **LangChain 기반 별도 RAG 스택**으로 “Dify 없이도 구성 가능”까지 학습
...
Dify에는 지식 베이스, 즉 내장 RAG가 있지만, 검색 품질이나 파이프라인 제어, 복수 앱 분리 같은 면에서 한계가 있어서 실무에서는 LangChain 기반 자체 RAG로 전환하는 경우가 많습니다. 이 강의에서는 Dify로 RAG 흐름을 이해하고, LangChain 기반 별도 RAG 스택으로 “Dify 없이도 구성 가능”한 부분까지 함께 다룹니다.
