...
• **본 강의 범위 제외**: n8n/, workflows/ (RAG·Dify·chat-gateway·chat-admin만 다룸)
...
RAG와 토픽별로는 rag 아래에 cointutor, drillquiz 폴더가 있고, 각각 해당 토픽의 RAG Backend YAML과 Ingestion Job·CronJob이 있습니다. rag/scripts/ingest.py는 MinIO에서 읽어서 LangChain으로 로드·분할·임베딩한 뒤 Qdrant에 넣는 인제스트 스크립트입니다. 저장소에는 n8n, workflows 같은 폴더도 있지만, 이 강의에서는 RAG·Dify·chat-gateway·chat-admin만 다룹니다. 이 구조를 알아두면 실습이나 커스터마이징 시 경로를 찾기 수월합니다.
